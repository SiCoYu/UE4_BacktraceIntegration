...
int FBacktraceIntegration::ScaledSize(int value)
{
    ...
    //UEditorEngine* ed = GEditor;
    //
    ////scale = FSlateApplication::Get().GetApplicationScale();
    ////if (window.IsValid() && window.Get()->GetViewport().IsValid())
    //if (ed && ed->GetActiveViewport())
    //{
    //      auto tb = SNew(STextBlock).Text(FText::FromString("Size Test"));
    //      auto box = SNew(SBox)[tb];
    //      auto ds = tb->GetDesiredSize();

    //      BACKTRACE_LOG(BacktraceLog, Warning, TEXT("DS: %f %f"), ds.X, ds.Y);
    //      FVector2D size;
    //      size = ed->GetActiveViewport()->GetSizeXY();
    //      auto sclass = UUserInterfaceSettings::StaticClass();
    //      auto def = GetDefault<UUserInterfaceSettings>(sclass);
    //      //scale = def->GetDPIScaleBasedOnSize(FIntPoint(size.X, size.Y));

    //      auto winapiScale = Backtrace::GetDPISettings();
    //      BACKTRACE_LOG(BacktraceLog, Warning, TEXT("WINAPI SCALE %f"), winapiScale);
    //}
    //else if (ed) {
    //      BACKTRACE_LOG(BacktraceLog, Warning, TEXT("ed exists"));
    //}

    //BACKTRACE_LOG(BacktraceLog, Warning, TEXT("%f"), scale);
